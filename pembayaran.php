...
<body>
    <div class="mt-2 py-2">
        ...
        <form action="" method="post">
            ...
            <?Php
            $SESSION['JumlahHarga'] = 0;

            if($_SESSION['DataBarang'] && !empty($_SESSION['DataBarang'])) {
                foreach ($_SESSION['DataBarang'] as $key ) {
                    $SESSION['JumlahHarga'] += $key['JumlahHarga'];
                }
                echo "<p>Total Harga : Rp. " . number_format($SESSION['JumlahHarga'], 2, ',', '.') . "</p>";
            }

            if (isset($_POST['Bayar'])) {
                $NominalUang = $_POST['NominalUang'];
                $SESSION['UangKembalian'] = $NominalUang - $_SESSION['JumlahHarga'];

                if ($NominalUang < $SESSION['JumlahHarga']) {
                    $KurangUang = $SESSION['JumlahHarga'] - $NominalUang;
